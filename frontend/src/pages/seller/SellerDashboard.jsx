...
// NOTE: assumed hook name, following the useGetMy* convention used by
// useGetMyStores / useGetMyProducts. If your seller-scoped orders hook is
// named differently (or lives in a different file), swap this one import —
// everything else below just consumes `orders` as an array.
import { useGetSellerAllOrders } from "../../api/order.js";
...
export default function SellerDashboard() {
  ...
  const { data: ordersRes, isLoading: ordersLoading } = useGetSellerAllOrders();
  ...
  const pendingProducts = products.filter(
    (p) => (p.approvalStatus || "pending").toLowerCase() === "pending"
  );

  const recentOrders = sortByCreatedAtDesc(orders).slice(0, 5);
  const recentProducts = sortByCreatedAtDesc(products).slice(0, 5);

  return (
    <div className="space-y-6 font-sans">
      <div>
        <h1 className="font-display font-semibold text-2xl text-ink">Dashboard Overview</h1>
        <p className="text-ink-muted text-sm mt-1">Manage your storefront activity and track sales.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        ...
        <StatCard label="Active Stores" value={stores.length} icon={Store} />
        <StatCard label="Listed Products" value={products.length} icon={Package} />
        <StatCard label="Stock Units" value={totalStock} icon={Layers} />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div className="xl:col-span-2 bg-surface border border-border rounded-lg p-5 hover:shadow-[0_4px_16px_rgba(28,2,33,0.08)] transition-shadow">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-ink-muted mt-0.5">Daily revenue across your stores.</p>
            </div>
        ...
        <div className="bg-surface border border-border rounded-lg p-5 hover:shadow-[0_4px_16px_rgba(28,2,33,0.08)] transition-shadow">
          ...
          <p className="text-xs text-ink-muted mb-4">Breakdown of your orders.</p>
      ...
      <div className="flex items-center gap-4">
        <div
          className={`p-3 rounded-md shrink-0 ${
            needsAttention ? "bg-danger-soft text-danger-text" : "bg-accent-soft text-accent-text"
          }`}
        >
          ...
          {orders.map((order) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <span className="font-semibold text-brand text-sm">${order.totalPrice?.toFixed(2)}</span>
                <span
...
function RecentProducts({ products }) {
  return (
    <div className="bg-surface border border-border rounded-lg overflow-hidden hover:shadow-[0_4px_16px_rgba(28,2,33,0.08)] transition-shadow">
      <div className="flex items-center justify-between px-4 py-3 border-b border-border">
        <h2 className="font-display font-semibold text-sm text-ink">Recently listed products</h2>
        <Link
          to="/seller/products"
          className="flex items-center gap-1 text-xs font-medium text-accent hover:text-accent-text transition-colors"
        >
      ...
      {products.length === 0 ? (
        <p className="text-ink-muted text-sm p-4">No products listed yet.</p>
      ) : (
        <div className="divide-y divide-border">
          {products.map((product) => (
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                {product.images?.[0]?.url ? (
                  <img
                    ...
                    alt={product.name}
                    className="w-8 h-8 object-cover rounded-md bg-surface-muted shrink-0"
                  />
                ) : (
                  <div className="w-8 h-8 rounded-md bg-surface-muted shrink-0" />
                )}
                <div className="min-w-0">
                  <p className="text-ink font-medium truncate">{product.name}</p>
                  <p className="text-ink-muted text-xs mt-0.5">
                    {(product.stock ?? 0) <= LOW_STOCK_THRESHOLD ? (
                      <span className="text-danger-text">{product.stock ?? 0} left</span>
                    ) : (
              ...
              <span className="font-semibold text-brand text-sm shrink-0">
